...
import type { FetchCallbackFunctionType, SubscribeOptions, UpdateNotification } from '../types';
...
class MqttConnection {
    ...
    async publish(message: string, options?: IClientPublishOptions): Promise<void> {
            ...
            if (this.client) {
                this.client.publish(this.topic, message, options, (error) => {
                    if (error) {
                        ...
                        this.logger.info('Published message to default topic:' + this.topic);
                        resolve();
                    }
                });
            } else {
    ...
    async subscribe(options: SubscribeOptions, onMessage: FetchCallbackFunctionType): Promise<void> {
        await this.ensureConnected();
        return await new Promise<void>((resolve, reject) => {
            if (this.client) {
                ...
                this.client.on('message', (topic, message) => {
                    this.logger.debug('MQTT message received on topic ' + topic, JSON.parse(message.toString()));
                    let updateNotification: UpdateNotification = JSON.parse(message.toString());
